...
- **Viewer is folder-first.** Its LUT column reconstructs the on-disk folder hierarchy and parent folders include every descendant LUT. The main preview sits above a lazy contact sheet whose cards render through the same `EditDocument` and `RenderEngine`, so choosing a look never relies on a second thumbnail colour pipeline.
