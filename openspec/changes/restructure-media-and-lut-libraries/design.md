...
Viewer's secondary column is vertically composed. A compact Media section at the top selects the active image. Below it, a LUT section is grouped into Folder, Collection, and Starred sources. A native draggable divider lets users allocate the available height between those regions while enforcing a usable minimum for each. Selecting either kind of local item updates the relevant Viewer content while the right-hand comparison and LUT-gallery split remains mounted. The former inline media filmstrip is removed because it duplicated the left Media chooser and inserted a third region between comparison and LUT Gallery. Video records are included in Media with an explicit browse-only/playback-unavailable presentation; the durable `MediaKind.video` boundary is retained so future playback can be added without another library migration.
...
The Manager table exposes this dedicated metadata as a `Brand` column between Folder and Input. This makes incomplete curation visible at list scale: `Unknown` remains explicit until the user assigns a confirmed vendor or Custom in the Inspector. The column never treats a folder name, filename prefix, or descriptive Tag as a confirmed brand.
...
The first implementation will test a persistent right-side metadata Inspector. A single selection exposes editable Name, Brand/Source, tags, Collection membership, folder, and Starred state. `Name` is the user-facing label for the display-name override; `Brand / Source` is the user-facing label for the dedicated Vendor/Custom/Unknown namespace while the persisted model may retain its existing `origin` key for compatibility. Tags are visible chips with direct add/remove actions. Removing a content-measured tag creates a per-record exclusion rather than mutating the shared measurement, so identical LUT files can be curated independently and the removal survives rescan. Adding the same text again clears that exclusion and records it as a user tag. Multiple selections expose common and mixed values plus explicit batch add/remove actions; fields without a safe batch meaning, such as Name, are disabled. Tag, folder move, Starred, Collection, and Remove actions live in this Inspector; the table does not duplicate them in a bottom action bar. This is a testable first interaction rather than a permanent visual constraint and may be refined after running-app review.
