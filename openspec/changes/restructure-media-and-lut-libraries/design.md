...
LUT Library discovery has three explicit levels. Its home is a vertically scrolling set of horizontal shelves, with a segmented grouping choice for Brand, Tag, or Collection. Tag is the initial grouping because measured local libraries can immediately present several useful rows even before users finish curating Brand or Collection metadata. The existing Folder/Collection/Starred sidebar remains a source scope: it limits the records used to construct shelves rather than becoming a second competing discovery control. Brand shelves use confirmed Origin metadata only, preserving Custom and Unknown instead of guessing from folder or filename. Tag shelves use the union of user-authored and measured tags, exclude internal `input:*` pipeline metadata that already has a dedicated Input field, and order broader populated rows first. Collection shelves use the existing local virtual Collections and omit empty intersections with the active source.

Opening a shelf heading or View All replaces the home with a complete searchable Grid for that shelf. Card activation from either home or Grid opens the same LUT Library detail rather than switching to Viewer or Manager. Back returns to the exact originating home or Grid context, restores the originating card/control selection, and returns keyboard and assistive-technology focus rather than dropping it when the Back control disappears. This adopts the reference's catalogue rhythm without copying its styling and without adding marketplace, creator, download, or remote content concepts.
