...
die inside it. Only `Sendable` values cross in — read off the `RenderEngining` protocol at HEAD:
`ImageSource`, `EditDocument`, `CubeLUT?`, `RenderScale`, `WorkingSpace`, `ExportFormat`, `CGFloat`.
Out: a `sending CGImage?`, a `Data`, a `HistogramData?`, a `RAWCapabilities?`.

_(This list previously read `EditDocument, ImageSource, WorkingSpace, RenderScale, LUTID, URL/Data`.
The asserted property — only `Sendable` values cross — was and is true; the enumeration had drifted.
`LUTID` does not cross the boundary at all: the whole `CubeLUT` does, and it is the largest value that
makes the trip.)_
...
| ~~9~~ | ~~Wire derive into the new state: register the derived LUT by ID, keep the scratch-file bookkeeping~~ | ✅ **done** — 230 tests; 19 mutations caught, 1 shown equivalent by inspection; fixed a **shipped** bug where a derived LUT never resolved (see below). ✅ **Tally re-measured under the corrected classifier and it stands exactly**: 19 caught, 1 control survived as designed, **0 skipped, 0 no-build, 0 no-tests** — a fully conclusive run, with the DNG present. The classifier that produced the original number was broken (it tested for "skipped" before "failed", so in a suite mixing RAW-gated with ordinary tests it could score a run by the wrong branch, and `SKIPPED` was excluded from the exit gate); the number it produced happened to be right, which is luck rather than evidence, and is why it was re-run rather than trusted |
